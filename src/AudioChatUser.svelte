...
<div class="user {user.id}" class:talking={user.talking} transition:fade>
  <div class="icon">
    {#if userIndex > -1}
      <img
        src={get($globalUserList[userIndex], "avatarLink.iconUrl", "")}
        alt={user.display}
      />
    {/if}
  </div>
  <div class="name">
    ...
    {#if userIndex > -1}
      <a
        href={"https://work.anthropocene-curriculum.org/new-message?username=" +
          get($globalUserList[userIndex], "username", "")}
        target="_blanks"
      >
        <svg
          width="13"
